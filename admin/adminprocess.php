...
class AdminProcess {
    ...
    function procDeleteUser() {
        ...
        /* Delete user from database */ else {
            $q = "DELETE FROM " . TBL_USERS . " WHERE prisijungimas = '$subuser'";
            $database->query($q);
            header("Location: " . $session->referrer);
        }
    }
    ...
    function procDeleteInactive() {
        ...
        $q = "DELETE FROM " . TBL_USERS . " WHERE trukme < $inact_time "
                . "AND lygis != " . ADMIN_LEVEL;
        $database->query($q);
        header("Location: " . $session->referrer);
    }
    ...
    function procDeleteBannedUser() {
        ...
        /* Delete user from database */ else {
            $q = "DELETE FROM " . TBL_BANNED_USERS . " WHERE prisijungimas = '$subuser'";
            $database->query($q);
            header("Location: " . $session->referrer);
        }
    }
