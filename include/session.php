...
class Session {
    ...
    function startSession() {
         ...
         */
        if (!$this->logged_in) {
            $this->username = $_SESSION['prisijungimas'] = GUEST_NAME;
    ...
    function checkLogin() {
        ...
        if (isset($_COOKIE['cookname']) && isset($_COOKIE['cookid'])) {
            $this->username = $_SESSION['prisijungimas'] = $_COOKIE['cookname'];
            $this->userid = $_SESSION['id'] = $_COOKIE['cookid'];
        }

        /* Username and userid have been set and not guest */
        if (isset($_SESSION['prisijungimas']) && isset($_SESSION['id']) &&
                $_SESSION['prisijungimas'] != GUEST_NAME) {
            /* Confirm that username and userid are valid */
            if ($database->confirmUserID($_SESSION['prisijungimas'], $_SESSION['id']) != 0) {
                /* Variables are incorrect, user not logged in */
                unset($_SESSION['prisijungimas']);
                unset($_SESSION['id']);
                return false;
            }

            /* User is logged in, set class variables */
            $this->userinfo = $database->getUserInfo($_SESSION['prisijungimas']);
            $this->username = $this->userinfo['prisijungimas'];
    ...
    function login($subuser, $subpass, $subremember) {
        ...
        $this->username = $_SESSION['prisijungimas'] = $this->userinfo['prisijungimas'];
        $this->userid = $_SESSION['id'] = $this->generateRandID();
        $this->userlevel = $this->userinfo['lygis'];
    ...
    function register($subuser, $subpass, $subemail) {
        ...
        if (!$subuser || strlen($subuser = trim($subuser)) == 0) {
            ...
            /* Check if username is not alphanumeric */ else if
        ...
        if (!$subpass) {
            ...
            /* Check if password is not alphanumeric */ else if
            /*(!eregi("^([0-9a-z])+$", ($subpass = trim($subpass)))) */
            (!preg_match("/^([0-9a-z])*$/", ($subpass = trim($subpass))))
    ...
    function editAccount($subcurpass, $subnewpass, $subemail) {
        ...
        /* Change Email */
        if ($subemail) {
            $database->updateUserField($this->username, "epastas", $subemail);
        }
